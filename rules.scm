...
(define-syntax կրկնել
  (քերականություն֊կանոն ()
    ((_ (i from to) գործողություն ...)
     (let loop((i from))
       ( when(< i to)
	  գործողություն ...
	  (loop (1+ i)))))))
...
(սահմանել֊քերականություն երբ
  (քերականություն֊կանոն ()
    ((_ պայման գործողություն1)
     (if պայման գործողություն1))))
 
;if else 
(սահմանել֊քերականություն եթե
  (քերականություն֊կանոն (ապա այլապես)
...
(սահմանել (ստանալ զանգված համար)
 (if (<= համար 0)
     (car զանգված)
     (ստանալ (cdr զանգված) (- համար 1))))
     
;length of list
(սահմանել֊քերականություն երկարություն
	(քերականություն֊կանոն ()
	((_ ցուցակ)
	(length ցուցակ))))
...
;equal?
(սահմանել֊քերականություն հավասար
	(քերականություն֊կանոն ()
	((հավասար արժեք1 արժեք2)
	(equal? արժեք1 արժեք2))))
...
(սահմանել (շրջել լիստ)
  (կատարել
  (սահմանել (շրջել1 լիստ հին)
    (եթե (null? լիստ) ապա
      հին
      այլապես (շրջել1 (cdr լիստ) (cons (car լիստ) հին))))
  (շրջել1 լիստ '())))
  
;append list
(սահմանել (ավելացնել . ցուցակ)
  (cond
    ((null? ցուցակ) '())
    ((null? (car ցուցակ)) (apply append (cdr ցուցակ)))
    (else (cons (caar ցուցակ) (apply append (cdar ցուցակ) (cdr ցուցակ))))))
    
    
;make matrix
(սահմանել (ստեղծել֊մատրից ա բ լիստ)
  (let outter ((ի (- ա 1)) (արդյունք '()))
    (եթե (= ի (- 1))
        ապա արդյունք
        այլապես (outter ( - ի 1) 
                (cons
                 (let inner ((ժ (- բ 1)) (տող '()))
                   (եթե (= ժ (- 1))
                        	ապա տող
                       	այլապես (inner (- ժ 1) (cons (ստանալ (ստանալ  լիստ ի) ժ) տող))))
                 արդյունք)))))
                 
;set item from two dimensional matrix
(սահմանել (տալ֊արժեք մատրից ա բ  արժեք)
	(կատարել
	(վերագրել ն (երկարություն մատրից)) 
	(վերագրել մ (երկարություն (ստանալ մատրից 0))) 
	(let outter ((ի (- ն 1)) (արդյունք '()))
    	(եթե (= ի (- 1))
      	  	ապա արդյունք
        		այլապես (outter ( - ի 1) 
                (cons
                 (let inner ((ժ (- մ 1)) (տող '()))
                   (եթե (= ժ (- 1))
                       	ապա տող
                       	այլապես (inner (- ժ 1) (cons (եթե (և (= ի ա) (= ժ բ)) ապա արժեք 
                       	այլապես (ստանալ (ստանալ  մատրից ի) ժ)) տող))))
                 	արդյունք))))))
                 
;set item from one dimensional list	
(սահմանել (տալ ցուցակ դիրք արժեք)
    (if (zero? դիրք)
        (set-car! ցուցակ արժեք)
        (տալ (cdr ցուցակ) (- դիրք 1) արժեք)))
